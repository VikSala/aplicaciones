...
class ThemeWebsite(Website):
    ...
    def dr_search(self, term, max_nb_chars, options, **kw):
        ...
        if product_limit:
            results['products'] = self.autocomplete(search_type='products_only', term=term, order='name asc', limit=product_limit, max_nb_chars=max_nb_chars, options=options)
        product_fuzzy_term = results['products'].get('fuzzy_search')

        if search_config.get('search_category') and not has_formulate:
            results['categories'] = self.autocomplete(search_type='product_categories_only', term=term, order='sequence, name, id', limit=5, max_nb_chars=max_nb_chars, options=options)
            ...
            if fuzzy_enabled:
                ...
                if category_fuzzy_term == product_fuzzy_term:
                    ...
                    all_results = self.autocomplete(search_type='products', term=term, order='sequence, name, id', limit=limit, max_nb_chars=max_nb_chars, options=options)
            ...
            if results.get('categories') and not results.get('categories').get('results_count'):
                category_result = self.autocomplete(search_type='category_synonyms', term=term, order='name asc', limit=5, max_nb_chars=max_nb_chars, options=options)
