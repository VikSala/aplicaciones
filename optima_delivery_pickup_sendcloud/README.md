...
## 18.0.0.8.0 — pruebas de puntos de recogida con Unstamped Letter

**Activación:** publica en la web el método técnico **Sendcloud Unstamped letter**
(de la **misma integración Sendcloud** que tus métodos de recogida). No marques
ese método como «requiere punto de servicio». El adaptador reconoce su nombre
o el del método Sendcloud asociado, sin codificar un ID remoto variable. Después
de publicarlo, selecciona **de nuevo** un punto de Correos, InPost, etc. en el
checkout. Si despublicas la carta, las **selecciones nuevas** usarán el método
real. La decisión queda guardada en cada selección existente; despublicar no
convierte albaranes de prueba ya confirmados en expediciones reales.

- La búsqueda del mapa, la verificación del punto y del bulto, y la resolución
  de la **tarifa PUDO real** se ejecutan como antes. En el pedido de venta se
  conserva el transportista elegido y el precio cobrado por ese método.
- Al confirmar, SOLO el transportista del albarán de salida se cambia al
  Unstamped Letter publicado. El albarán muestra el método que se tarifó y el
  utilizado para la etiqueta de prueba. El punto validado permanece en los
  snapshots de Optima; los campos PUDO del conector OCA se vacían para no
  intentar enviar un `service_point_id` junto con una carta ordinaria.
- `send_to_shipper` comprueba que el transportista de prueba sigue siendo el
  indicado en el snapshot y bloquea cualquier intento de enviar con el método
  PUDO de pago. La llamada de creación de etiqueta y la impresión siguen el
  flujo del módulo `delivery_sendcloud_oca` sin modificaciones.
- No se actualiza ni se añade al pedido una segunda línea de gastos de envío
  con el coste devuelto para Unstamped Letter. El precio original del checkout
  no cambia.
- Si no hay Unstamped Letter publicado para esa web, empresa e integración,
  no se activa el modo de prueba y se conserva el funcionamiento habitual.
  Si hay varias cartas publicadas o la integración es distinta, se bloquea la
  selección en lugar de enviar un método posiblemente de pago.

**Alcance:** esta simulación verifica la selección PUDO en Odoo, la persistencia,
la confirmación, la comunicación de la etiqueta de carta con Sendcloud y su
impresión. **No** verifica el registro de una expedición PUDO real en Sendcloud,
la entrega al punto por Correos/InPost, el tracking de ese transportista ni
las devoluciones. La etiqueta de carta **no sirve para transportar el paquete**.
Sendcloud indica que pueden cobrarse tasas de procesamiento por estas etiquetas;
consulta las condiciones de tu plan antes de generar muchas.

**Prueba sugerida:** en un entorno de Odoo de pruebas, publica la carta,
selecciona un punto de Correos o InPost, verifica precio y transportista en el
pedido, confirma y comprueba que el albarán marca «etiqueta de prueba» y usa
Unstamped Letter. Pulsa «Enviar al transportista», comprueba en Sendcloud que
la etiqueta generada es una carta no franqueada y descarga/imprime la etiqueta.
Después despublica la carta, selecciona un punto en otro pedido **sin pulsar
Enviar al transportista** y comprueba que el modo de prueba no está activo.
No uses números de albarán de prueba que luego reutilizarás en producción:
Sendcloud trata los números de referencia como identificadores idempotentes.
