...
class DeliveryCarrier(models.Model):
    ...
    def gls_asm_tracking_state_update(self, picking):
        ...
        digitalizaciones = []
        if not picking.carrier_id.gls_is_pickup_service:
            # GLS recommends using the barcode returned by GrabaServicios for
            # GetExpCli when available. It identifies the original shipment
            # unambiguously (important when a return shipment also exists).
            tracking_reference = (
                picking.gls_asm_public_tracking_ref or picking.carrier_tracking_ref
            )
            tracking_info = gls_request._get_tracking_states(tracking_reference)
